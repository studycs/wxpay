...
class WxPayApi
{
     ...
     * @throws WxPayException
     */
	public static function bizpayurl($config, $inputObj)
	{
	...
	public static function notify($config, $callback, &$msg)
	{
		...
		try{
			$result = WxPayNotifyResults::Init($config,$stream);
	...
	private static function reportCostTime($config, $url, $startTimeStamp, $data)
	{
		...
		//如果仅失败上报
		if($reportLevenl == 1 && array_key_exists("return_code", $data) && $data["return_code"] == "SUCCESS"
